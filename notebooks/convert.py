...
CACHE_PATH = os.path.join(os.path.dirname(__file__), "cls_cache.json")
...
def load_cls_cache(path=CACHE_PATH):
...
def save_cls_cache(cache, path=CACHE_PATH):
...
def process_file(args):
    """Process a single .far file and generate visualization."""
    file_name, folder, cls_cache = args
    file = os.path.join(folder, file_name)
    print(f"Loading file: {file}")

    try:
        # Create pipeline in each process to avoid multiprocessing issues

        reader = FaroSceneGraphReader(file, cls_cache)
        scene = reader.parse()
        print(f"Processed file: {file_name}")
        # print(f"File: {file_name}, Number of polycurves: {len(scene['scene_items'].get('polycurve', []))}, Number of polylines: {len(scene['scene_items'].get('polyline', []))}")
        ...
        for item in scene["roadway"]:
            ...
            if item.get("closed", False):
                ax.plot([xs[-1], xs[0]], [ys[-1], ys[0]], color="black")
                ax.fill(xs, ys, color="lightgray", alpha=0.5)

        ax.set_aspect("equal", "box")

        plt.title(f"FARO Scene Visualization: {file_name}")
...
if __name__ == "__main__":
    base_cache = load_cls_cache()
    with Manager() as manager:
        cls_cache = manager.dict(base_cache)

        # Create arguments for each file
        file_args = [(file_name, folder, cls_cache) for file_name in far_files]

        # Use multiprocessing to process files in parallel
        # Use all available CPUs minus 1 to leave some resources for the system
        # num_processes = max(1, cpu_count() - 1)
        num_processes = 128
        print(f"Processing {len(far_files)} files using {num_processes} processes...")

        with Pool(processes=num_processes) as pool:
            results = list(
                tqdm(
                    pool.imap_unordered(process_file, file_args),
                    total=len(file_args),
                    desc="Processing files",
                )
            )

        save_cls_cache(dict(cls_cache))
