...
def load_checkpoint(path, model, optimizer, scheduler):
    ...
    # If the checkpoint was saved under a different base_lr than the current config, overwrite it
    if any(base != CFG.lr for base in scheduler.base_lrs):
        old_base = list(scheduler.base_lrs)
        old_lr = [g["lr"] for g in optimizer.param_groups]
        scheduler.base_lrs = [CFG.lr] * len(scheduler.base_lrs)
        for group, lam in zip(optimizer.param_groups, scheduler.lr_lambdas):
            group["initial_lr"] = CFG.lr
            group["lr"] = CFG.lr * lam(scheduler.last_epoch)
        new_lr = [g["lr"] for g in optimizer.param_groups]
        print(
            f"OVERWRITING checkpoint LR state: the checkpoint was saved under "
            f"base_lr={old_base[0]}, but this run configures lr={CFG.lr}. "
            f"The restored step has {new_lr} instead of the {old_lr}. "
        )
        ...
        for step, batch in enumerate(loader):
            ...
            if is_train:
                ...
                if WANDB_ON and step % CFG.log_every == 0:
                    ...
                    # The lr the optimizer ACTUALLY uses, not what the config asked for —
                    # the two can differ (e.g. a resumed scheduler carrying the old run's
                    # base_lrs), and every spike so far has been lr-dependent.
                    if scheduler is not None:
